...
describe('OrderService', () => {
  ...
  describe('createOrder', () => {
    beforeEach(() => {
      ...
      listenNotifyService.mockClear();

      it('shold create a new order', async () => {
        const trakingService = new TrakingService(new InMemoryTrakingRepository());
    ...
    describe('findAllOrdersThatNotHaveBeenDelivered', () => {
      ...
      it('should be to find all orders that not have been delivered', async () => {
        ...
        const results = await orderService.findAllOrdersThatNotHaveBeenDelivered();
    ...
    describe('refreshOrderTraking', () => {
      ...
      it('should ble able to refresh order traking if order already have trakings registred', async () => {
        ...
        vi.spyOn(fakeDeliveryServiceProvider, 'getAllTrakingByTrakingCode').mockResolvedValue([
          {
            message: faker.lorem.sentence(),
            date: yesterday,
          },
        ...
        vi.spyOn(fakeDeliveryServiceProvider, 'getMoreRecentTrakingOrder').mockResolvedValue({
          traking: {
        ...
        const traking = await trakingService.findMoreRecentTrakingByOrderId(order.id);
      ...
      it('it shold be able to refresh order traking if order no have trakings registred yet', async () => {
        ...
        const traking = await trakingService.findMoreRecentTrakingByOrderId(order_id);
      ...
      it('should not be able to refresh order traking if order not exists', async () => {
        ...
        await expect(orderService.refreshOrderTraking(fake_id)).rejects.toBeInstanceOf(OrderNotFoundError);
      });
      ...
      it('not should be able to refresh order traking if deliveryServiceProvider not return trakings', async () => {
        ...
        vi.spyOn(fakeDeliveryServiceProvider, 'getMoreRecentTrakingOrder').mockResolvedValue(null);

        const spyRefreshOrderTraking = vi.spyOn(orderService, 'refreshOrderTraking');
